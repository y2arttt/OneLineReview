...
- **예외 전역 처리 / 커스텀 예외 처리**
  ...
  또한 예외 처리를 관리하기 쉽게 커스텀 예외를 만들었습니다.
  커스텀 예외만 사용할지, 예외 코드도 함께 사용할지 고민하다가 예외 코드도 사용하기로 했습니다.
  나중에 예외가 많아질수록 어떤 예외인지 빠르게 알기 위해선 세분화하는 게 나을 것 같다는 생각 때문입니다.
...
- **도서 정보 받아오기**
  ...
  모든 도서 정보를 db에 저장하는 것은 비효율적이라 판단해 네이버 API를 이용해서 db에 저장하는 방식을 사용했습니다.
  사용자가 도서 상세 정보를 들어갈 때, **만약 db에 존재하지 않으면** API를 불러와 저장했습니다.
