# 한줄 리뷰
한줄 리뷰는 java,spring으로 만든 짧은 독서 리뷰를 위한 웹 사이트 입니다.
문학 동아리 부원들과 함께 독서평을 하기 위해 만들게 되었습니다.
...
웹사이트에 접속시 유저의 리뷰들을 볼 수 있습니다.
클릭시 리뷰를 자세히 볼 수 있습니다.
최근 순으로 정렬되며 한 번에 10개씩 보여집니다.
스크롤을 내리면 추가로 10개가 보여집니다.
...
상단 도서검색 바를 통해 검색할 수 있습니다.
네이버 API를 기반으로 정보를 불러옵니다.
도서를 클릭시 정보 페이지로 이동합니다.
...
도서 정보와 도서의 리뷰를 불러옵니다. 만약 자신의 리뷰가 있을 경우 자신의 리뷰를 보여주며, 그렇지 않을 경우 리뷰 작성 버튼을 보여줍니다.
...
- **게시글/댓글 유저 정보 확인**
  ...
  게시글을 수정하거나 삭제시 소유 유저와 요청한 유저가 일치한지 확인하기 위하여 인터셉터를 사용하였다.
...
- **예외 전역 처리 / 커스텀 예외 처리**
  ...
  컨트롤러에서 오류가 발생시 dispatcherServlet을 거쳐 was까지 전달된다. 이후 에러 페이지로 다시 요청을 보내게 되는데,
  다시 거쳐야하는 작업이 많으므로 비효율적이다.
  이를 해결하기 위하여 ExceptionHandler를 사용하면 dispatcherServlet까지 예외를 전달하지 않아도 된다.
  이때 예외 처리를 쉽게 하기위해 ControllerAdvice빈에 ExceptionHandler를 만들어 관리하였다.
  
  또한 예외 처리를 관리하기 쉽게 커스텀 예외를 만들었다.
  커스텀 예외만 사용할지, 예외 코드도 함께 사용할지 고민하다가 예외 코드도 사용하기로 했다.
  나중에 예외가 많아질수록 어떤 예외인지 빠르게 알기 위해선 세분화하는게 나을 것 같다는 생각 때문이다.

- **예외 로깅**
  
  예외 발생시 어떤 예외가 발생했는지 관리하기 위하여 로깅 aop를 만들었다.
  예외에 대한 로그 기록이라는 공통된 관심사를 처리하기 위함이다.

- **도서 정보 받아오기**

  모든 도서 정보를 db에 저장하는것은 비효율적이라 판단해 네이버API를 이용해서 db에 저장하는 방식을 사용했다.
  사용자가 도서 상세 정보를 들어갈 떄, **만약 db에 존재하지 않으면** API를 불러와 저장했다.
